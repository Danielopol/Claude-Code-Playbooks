...
Two binaries must be on `PATH` before anything here works:

- **`yt-dlp`** — downloads the video and pulls native captions.
- **`ffmpeg`** (with `ffprobe`) — extracts frames and, when Whisper is needed, a mono 16 kHz audio clip.

Install commands by platform:

```bash
# macOS (Homebrew) — the bundled installer runs this automatically
...
**Optional Whisper API key.** Native captions cover most public videos for free. The Whisper fallback only fires when a video has no caption track (typical for local files, TikToks, some Vimeos, caption-less YouTube uploads). Two backends:

| Capability | What you need | Cost |
|------------|---------------|------|
| Download + native captions | `yt-dlp` + `ffmpeg` | Free |
| Whisper fallback (preferred) | Groq API key (`whisper-large-v3`) — console.groq.com/keys | Cheap, fast |
| Whisper fallback (alt) | OpenAI API key (`whisper-1`) — platform.openai.com/api-keys | Standard pricing |
| Disable Whisper entirely | `--no-whisper` | Free, frames-only when no captions |

Keys live in `~/.config/watch/.env` (mode `0600`). The script prefers Groq when both are set.

**Bundled scripts.** The `python3` commands below all run scripts shipped with the skill. Install them one of these ways:
...
# Any Agent Skills host (Codex, Cursor, Copilot, Gemini CLI, ...)
...
Scripts installed: `scripts/watch.py` (entry point), `scripts/download.py` (yt-dlp wrapper), `scripts/frames.py` (ffmpeg frame extraction), `scripts/transcribe.py` (caption selection + Whisper orchestration), `scripts/whisper.py` (Groq / OpenAI clients), `scripts/setup.py` (preflight + installer). Review scripts before first use to verify behavior.

## Resolve `SKILL_DIR` (do this before any command)

Every `python3 ...` command below runs a bundled script under `SKILL_DIR/scripts/`. Set `SKILL_DIR` to the **absolute path of the directory containing the skill's SKILL.md** — your harness tells you that path in the Read result. The scripts are always a direct sibling of that file (`SKILL_DIR/scripts/watch.py`), in every install layout:
...
- User pastes a video URL (YouTube, Vimeo, X, TikTok, Instagram, Loom, Twitch clip, most yt-dlp-supported sites) and asks about it.
...
Frame selection can still surface near-identical frames: a screen recording that holds one slide for 90 seconds produces a dozen, each billed as a separate image. A dedup pass drops them before frames reach Claude. It runs by default on every frame mode (`--no-dedup` turns it off):
