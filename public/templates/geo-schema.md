...
Audit, validate, and generate schema.org JSON-LD markup optimized for AI discoverability. Structured data is the primary machine-readable signal that tells AI systems what an entity IS, what it does, and how it connects to other entities.

Schema markup has traditionally been about earning Google rich results. Its role in GEO is different: structured data is how AI models understand and trust your entity. A complete entity graph in structured data increases citation probability across AI search platforms.
...
**Fetching caveat:** WebFetch converts HTML to markdown and strips `<head>` content, which removes JSON-LD blocks. Retrieve the raw HTML instead — `curl -sL <url>` from Bash works, or use a fetch script that returns unmodified source. The full geo-seo-claude toolkit ships a `fetch_page.py` helper whose output includes a `structured_data` array with all parsed JSON-LD blocks; if you have the suite installed, use that.
...
### Scan for Microdata
Look for elements with `itemscope`, `itemtype`, and `itemprop` attributes. Map the hierarchy of nested items. Microdata is harder for AI crawlers to parse than JSON-LD. Flag a recommendation to migrate to JSON-LD if Microdata is the only format found.
...
### FAQPage
**Status as of 2024**: Google restricts FAQ rich results to government and health sites. The FAQPage schema still serves GEO purposes — AI platforms parse FAQ structured data for question-answer extraction. Implement it for AI readability even though rich results may not appear.
...
The `speakable` property marks specific sections of content as suitable for voice and AI assistant consumption. Add to Article or WebPage schemas.
