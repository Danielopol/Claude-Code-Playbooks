...
**Status:** schema defined here. Blocks are added iteratively. Do not freelance new blocks without following this schema.
...
- [ ] **Bento Background Diversity**: at least 2-3 bento cells have real visual variation (image, gradient, pattern), not all white-on-white text cells?
...
The sections below are vendored reference content. They give real install commands, real canonical doc links, and real working starter snippets for each design system named in Section 2. Use them to ground decisions in production reality, not training-data fiction.
