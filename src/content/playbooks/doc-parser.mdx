...
seoHook: "Your PDF has 200 pages with tables that span columns, figures with captions, and headers that break across pages. Copy-paste destroys the formatting. OCR misses half the tables. You need the structure, not just the text."
targetAudience: "data engineers extracting structured data from PDFs, researchers parsing academic papers at scale, legal teams processing document archives, analysts converting reports into usable data, developers building document processing pipelines"
exampleUseCase: "\"Parse this 150-page financial report and extract all tables\" → Structured output preserving every table with headers, multi-column layouts correctly merged, figures extracted with captions, and the full document hierarchy intact — ready for downstream analysis"
