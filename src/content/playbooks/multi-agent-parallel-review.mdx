...
seoHook: "One reviewer catches typos but misses security holes. Another spots architectural issues but ignores documentation gaps. Sequential reviews take days and each reviewer only sees their own dimension. You need all perspectives simultaneously, not one at a time."
targetAudience: "engineering teams wanting thorough multi-dimensional code review, tech leads reviewing critical PRs before production deploys, security-conscious teams adding automated audit to their review process, open-source maintainers handling high volumes of contributions, teams establishing consistent review quality standards"
exampleUseCase: "\"Review this PR from every angle before we merge\" → Parallel reports from 5 specialized agents: code quality (3 issues), security audit (1 vulnerability), performance (2 optimizations), documentation (4 gaps), and test coverage (2 untested paths) — all in under 2 minutes"
