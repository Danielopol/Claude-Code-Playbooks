...
seoHook: "Your Salesforce instance has 200 custom fields nobody uses, reports that don't match reality, and permission sets that give everyone access to everything. A Salesforce admin costs $150K/year but you just need answers: how to build the right workflow, configure the report, or fix the validation rule that's blocking your team."
targetAudience: "Salesforce admins solving configuration challenges without developer support, sales ops managers building reports and dashboards for leadership, revenue operations teams optimizing CRM workflows and automation, small companies managing Salesforce without a dedicated admin, teams migrating to Salesforce from another CRM"
exampleUseCase: "\"Help me configure Salesforce for our new lead routing process\" → Configuration guide: workflow rules for round-robin lead assignment, validation rules for required fields at each stage, custom report type for pipeline visibility, dashboard layout with key sales metrics, and permission set configuration for role-based access"
