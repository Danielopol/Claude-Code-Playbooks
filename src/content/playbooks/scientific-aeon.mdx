---
title: "Scientific Aeon"
description: "This skill should be used for time series machine learning tasks including classification, regression, clustering, forecasting, anomaly detection, segmentation, and similarity search. Use when working with temporal data, sequential patterns, or ti..."
seoHook: "Your time series data needs more than a line chart and a moving average. Classification, anomaly detection, forecasting, and segmentation on temporal data require specialized ML approaches that standard tools don't handle well. Aeon provides purpose-built algorithms for every time series ML task — if you know which one to use and how to configure it."
targetAudience: "data scientists working with sensor data and IoT time series, ML engineers building time series classification and forecasting models, researchers analyzing temporal patterns in scientific data, operations teams detecting anomalies in monitoring data, quantitative analysts working with financial time series"
exampleUseCase: "\"Classify and forecast anomalies in our sensor data\" → Time series ML pipeline: data preprocessing with appropriate resampling, classification model selection from aeon's algorithm library, anomaly detection with configurable sensitivity, 30-day forecast with confidence intervals, and performance evaluation with appropriate temporal cross-validation"
...
tags: ["scientific", "claude-code", "aeon", "machine-learning", "statistics", "visualization", "database", "genomics"]
...
```bash
mv ~/Downloads/CLAUDE.md ~/Projects/aeon/
```
