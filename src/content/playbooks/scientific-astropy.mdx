...
seoHook: "Astronomical data comes in FITS files with WCS coordinates, peculiar time systems, and physical units that don't play nice with standard Python tools. Astropy handles celestial coordinate transforms, cosmological calculations, FITS I/O, and unit conversions — the foundational toolkit that every astronomical analysis depends on."
targetAudience: "astronomy PhD students processing observational data for the first time, astrophysics researchers building analysis pipelines for survey data, observational astronomers working with FITS files and WCS projections, cosmologists running distance and volume calculations, citizen scientists analyzing publicly available astronomical datasets"
exampleUseCase: "\"Process FITS images from our telescope survey and compute source coordinates\" → Astropy pipeline: FITS file reading with header parsing, WCS coordinate transformation from pixel to sky coordinates, source catalog cross-matching with proper angular separation, cosmological distance calculations for redshift catalog, and unit-aware photometry computations"
...
tags: ["scientific", "claude-code", "astropy", "statistics", "visualization", "database", "protein", "writing"]
...
```bash
mv ~/Downloads/CLAUDE.md ~/Projects/astropy/
```
