---
title: "Scientific Benchling Integration"
...
seoHook: "Your lab's DNA constructs are registered in Benchling, inventory is tracked there, and ELN entries document every experiment — but pulling data out for analysis means manual exports and copy-paste. Programmatic Benchling integration lets you query registries, automate workflows, and sync lab data with your computational pipelines."
targetAudience: "lab managers automating Benchling data workflows, biotech scientists integrating Benchling registry with analysis pipelines, bioinformaticians building automated data extraction from Benchling ELN, LIMS administrators customizing Benchling workflows via API, research teams building Benchling Apps for specialized lab processes"
exampleUseCase: "\"Automate extraction of our construct library data from Benchling\" → Integration pipeline: API connection to Benchling registry, batch query for DNA and protein entries with custom schema fields, inventory status check across all locations, ELN entry data extraction for experiment metadata, and structured export for downstream computational analysis"
...
tags: ["scientific", "claude-code", "benchling-integration", "bioinformatics", "visualization", "database", "protein", "genomics"]
...
```bash
mkdir -p ~/Projects/benchling-integration
```
...
```bash
mv ~/Downloads/CLAUDE.md ~/Projects/benchling-integration/
```

### Step 3: Start Claude Code

```bash
cd ~/Projects/benchling-integration
claude
```
