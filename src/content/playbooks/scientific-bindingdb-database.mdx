---
title: "Scientific Bindingdb Database"
description: "Query BindingDB for measured drug-target binding affinities (Ki, Kd, IC50, EC50). Search by target (UniProt ID), compound (SMILES/name), or pathogen. Essential for drug discovery, lead optimization, polypharmacology analysis, and structure-activit..."
seoHook: "You need binding affinity data for your drug target but it's scattered across thousands of papers. BindingDB aggregates measured Ki, Kd, IC50, and EC50 values from the literature into a queryable database — the question is knowing how to search by target, compound, or pathogen and filter for the assay conditions that match your use case."
targetAudience: "medicinal chemists evaluating SAR for lead compound series, computational chemists building QSAR models with experimental binding data, drug discovery scientists identifying known binders for novel targets, pharmacology researchers comparing binding profiles across target families, graduate students learning structure-activity relationships from real data"
exampleUseCase: "\"Query BindingDB for all known inhibitors of our kinase target with IC50 data\" → Binding data package: all compounds with measured IC50 values for the target, filtered by assay conditions, structure-activity relationship analysis showing key pharmacophore features, potency distribution across compound classes, and comparison with selectivity data against related kinases"
category: "academic-research"
difficulty: "beginner"
timeToSetup: "5 minutes"
author: "K-Dense AI"
sourceUrl: "https://github.com/K-Dense-AI/claude-scientific-skills/tree/main/scientific-skills/bindingdb-database"
tags: ["scientific", "claude-code", "bindingdb-database", "bioinformatics", "machine-learning", "chemistry", "database", "protein"]
...
```bash
mkdir -p ~/Projects/bindingdb-database
```
...
```bash
mv ~/Downloads/CLAUDE.md ~/Projects/bindingdb-database/
```

### Step 3: Start Claude Code

```bash
cd ~/Projects/bindingdb-database
claude
```
