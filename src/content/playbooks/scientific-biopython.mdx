...
seoHook: "You need to parse 500 GenBank files, run BLAST on a batch of sequences, and build a phylogenetic tree — and doing it manually in the NCBI web interface would take a week. Biopython handles sequence manipulation, file parsing, NCBI queries, and phylogenetics programmatically, turning one-at-a-time bioinformatics into automated pipelines."
targetAudience: "molecular biologists automating sequence analysis workflows, bioinformaticians building custom analysis pipelines, graduate students processing genomic data for their research, microbiology researchers running batch BLAST and sequence comparisons, computational biologists integrating NCBI data access into their tools"
exampleUseCase: "\"Build a pipeline to analyze 200 bacterial genome sequences\" → Biopython workflow: batch FASTA/GenBank file parsing, programmatic BLAST search against NCBI nr database, multiple sequence alignment, phylogenetic tree construction with bootstrap support, and automated NCBI Entrez queries for annotation metadata"
...
tags: ["scientific", "claude-code", "biopython", "bioinformatics", "visualization", "database", "protein", "genomics"]
...
```bash
mv ~/Downloads/CLAUDE.md ~/Projects/biopython/
```

### Step 3: Start Claude Code

```bash
cd ~/Projects/biopython
claude
```
