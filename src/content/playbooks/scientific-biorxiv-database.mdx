---
title: "Scientific Biorxiv Database"
...
seoHook: "New preprints in your field drop daily on bioRxiv and you can't keep up by manually browsing. Programmatic access to the bioRxiv API lets you search by keywords, authors, date ranges, and categories — build automated literature monitoring that alerts you to relevant papers before your competitors cite them."
targetAudience: "researchers monitoring preprint literature in fast-moving fields, graduate students conducting comprehensive literature reviews, lab PIs tracking competitor publications and new methods, journal clubs automating weekly paper discovery, bioinformaticians building literature surveillance pipelines"
exampleUseCase: "\"Search bioRxiv for all CRISPR-related preprints from the last 3 months in genomics\" → Literature search: filtered preprint list with metadata (title, authors, abstract, DOI), sorted by relevance and date, downloadable PDFs for top matches, keyword frequency analysis across results, and a structured summary table for lab meeting review"
...
tags: ["scientific", "claude-code", "biorxiv-database", "bioinformatics", "machine-learning", "database", "protein", "genomics"]
...
```bash
mkdir -p ~/Projects/biorxiv-database
```
...
```bash
mv ~/Downloads/CLAUDE.md ~/Projects/biorxiv-database/
```

### Step 3: Start Claude Code

```bash
cd ~/Projects/biorxiv-database
claude
```
