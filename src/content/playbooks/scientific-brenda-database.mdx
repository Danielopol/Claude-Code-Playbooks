---
title: "Scientific Brenda Database"
...
seoHook: "You need Km and kcat values for an enzyme across 15 organisms but the data is buried in thousands of papers. BRENDA is the world's most comprehensive enzyme database — but its SOAP API has a steep learning curve. Programmatic access lets you batch-retrieve kinetic parameters, reaction equations, and organism-specific data for metabolic modeling."
targetAudience: "biochemists collecting kinetic parameters for enzyme characterization, metabolic engineers building flux balance models with literature enzyme data, systems biologists parameterizing metabolic pathway simulations, pharmaceutical researchers studying enzyme inhibition across species, graduate students compiling enzyme data for their thesis research"
exampleUseCase: "\"Retrieve kinetic parameters for all enzymes in the glycolysis pathway\" → Enzyme data package: Km and kcat values for each enzyme across human, mouse, and E. coli, substrate specificity data, reaction equations with EC numbers, organism-specific isoform information, and formatted output ready for metabolic flux modeling"
...
tags: ["scientific", "claude-code", "brenda-database", "chemistry", "visualization", "database", "protein", "genomics"]
...
```bash
mkdir -p ~/Projects/brenda-database
```
...
```bash
mv ~/Downloads/CLAUDE.md ~/Projects/brenda-database/
```

### Step 3: Start Claude Code

```bash
cd ~/Projects/brenda-database
claude
```
...
- **Read the docs**: Check the official brenda-database documentation for latest API changes
