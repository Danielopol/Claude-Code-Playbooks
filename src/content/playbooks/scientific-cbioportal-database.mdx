---
title: "Scientific Cbioportal Database"
description: "Query cBioPortal for cancer genomics data including somatic mutations, copy number alterations, gene expression, and survival data across hundreds of cancer studies. Essential for cancer target validation, oncogene/tumor suppressor analysis, and p..."
seoHook: "You want to know how often TP53 is mutated across 30 cancer types and whether it correlates with survival — but downloading individual TCGA datasets and parsing them yourself takes weeks. cBioPortal aggregates cancer genomics data from hundreds of studies into a queryable API for somatic mutations, CNAs, expression, and clinical outcomes."
targetAudience: "cancer researchers validating targets across tumor types, translational scientists analyzing mutation frequency and clinical correlations, oncology drug developers assessing patient stratification biomarkers, graduate students in cancer biology conducting pan-cancer analyses, bioinformaticians building cancer genomics pipelines"
exampleUseCase: "\"Query mutation frequency and survival data for KRAS across all pancreatic cancer studies\" → Cancer genomics analysis: KRAS mutation spectrum with variant frequencies, co-mutation patterns with other driver genes, survival curves stratified by mutation status, copy number alteration data, and cross-study comparison of mutation rates"
category: "academic-research"
difficulty: "beginner"
timeToSetup: "5 minutes"
author: "K-Dense AI"
sourceUrl: "https://github.com/K-Dense-AI/claude-scientific-skills/tree/main/scientific-skills/cbioportal-database"
tags: ["scientific", "claude-code", "cbioportal-database", "bioinformatics", "database", "protein", "genomics", "writing"]
...
```bash
mkdir -p ~/Projects/cbioportal-database
```
...
```bash
mv ~/Downloads/CLAUDE.md ~/Projects/cbioportal-database/
```

### Step 3: Start Claude Code

```bash
cd ~/Projects/cbioportal-database
claude
```
