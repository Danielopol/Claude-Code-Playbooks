---
title: "Scientific Cellxgene Census"
description: "Query the CELLxGENE Census (61M+ cells) programmatically. Use when you need expression data across tissues, diseases, or cell types from the largest curated single-cell atlas. Best for population-scale queries, reference atlas comparisons. For ana..."
seoHook: "You need single-cell expression data for T-cells across 20 tissues but collecting, processing, and annotating that data from individual studies would take months. The CELLxGENE Census provides programmatic access to 61M+ cells from curated studies — query by tissue, disease, cell type, or gene and get analysis-ready data in minutes."
targetAudience: "single-cell biologists needing reference data across tissues and diseases, computational biologists building cell type classifiers with atlas-scale training data, immunologists comparing cell states across conditions, drug discovery researchers profiling target expression across cell types, bioinformaticians querying population-scale single-cell data for benchmarking"
exampleUseCase: "\"Query expression of our drug target gene across all immune cell types in the census\" → Census query results: gene expression data across immune cell subtypes from 61M+ cells, tissue-level expression comparison, disease vs. healthy differential expression, cell type specificity scoring, and AnnData-formatted output ready for downstream analysis"
...
tags: ["scientific", "claude-code", "cellxgene-census", "bioinformatics", "machine-learning", "visualization", "database", "protein"]
...
```bash
mkdir -p ~/Projects/cellxgene-census
```
...
```bash
mv ~/Downloads/CLAUDE.md ~/Projects/cellxgene-census/
```

### Step 3: Start Claude Code

```bash
cd ~/Projects/cellxgene-census
claude
```
...
- **Read the docs**: Check the official cellxgene-census documentation for latest API changes
