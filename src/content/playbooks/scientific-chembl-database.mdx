---
title: "Scientific Chembl Database"
...
seoHook: "You need all known bioactive compounds for your drug target with IC50 data, but manually searching the literature would take months. ChEMBL curates 2M+ compounds with bioactivity data from medicinal chemistry publications — the API lets you search by structure, target, or property and build SAR datasets in minutes instead of months."
targetAudience: "medicinal chemists building structure-activity relationship datasets, computational chemists training QSAR models with ChEMBL bioactivity data, drug discovery scientists identifying chemical starting points for new targets, pharmacologists comparing compound selectivity profiles, cheminformatics researchers building compound libraries from curated data"
exampleUseCase: "\"Find all known inhibitors of JAK2 with sub-micromolar IC50\" → ChEMBL query results: filtered compound set with structures and bioactivity values, SAR analysis showing key structural features for potency, selectivity data against JAK family members, drug-likeness assessment per compound, and formatted dataset ready for computational modeling"
...
tags: ["scientific", "claude-code", "chembl-database", "bioinformatics", "chemistry", "visualization", "database", "protein"]
...
```bash
mkdir -p ~/Projects/chembl-database
```
...
```bash
mv ~/Downloads/CLAUDE.md ~/Projects/chembl-database/
```

### Step 3: Start Claude Code

```bash
cd ~/Projects/chembl-database
claude
```
...
- **Read the docs**: Check the official chembl-database documentation for latest API changes
