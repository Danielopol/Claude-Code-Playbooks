...
seoHook: "Quantum circuits that work perfectly in simulation fail on real hardware because noise modeling wasn't part of the design. Cirq is Google's framework for building noise-aware quantum circuits — designed specifically for Google Quantum AI hardware with built-in noise characterization, device-specific optimization, and hardware execution support."
targetAudience: "quantum computing researchers targeting Google quantum hardware, physics PhD students learning quantum circuit design, quantum algorithm developers building noise-aware implementations, quantum characterization experimentalists running device benchmarks, researchers transitioning quantum algorithms from theory to hardware execution"
exampleUseCase: "\"Design a variational quantum eigensolver circuit for a small molecule using Cirq\" → Cirq implementation: parameterized ansatz circuit, noise model calibrated to current Google hardware, qubit layout optimized for device topology, simulation with and without noise comparison, and hardware execution configuration for Google Quantum AI processors"
...
tags: ["scientific", "claude-code", "cirq", "statistics", "visualization", "database", "protein", "genomics"]
createdAt: "2026-03-06"
---

## What This Does

A scientific skill for cirq workflows with Claude Code.
...
```bash
mv ~/Downloads/CLAUDE.md ~/Projects/cirq/
```
