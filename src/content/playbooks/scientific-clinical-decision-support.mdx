---
title: "Scientific Clinical Decision Support"
...
seoHook: "Clinical decisions need evidence-based support documents that integrate patient cohort data, biomarker stratification, and treatment guidelines — but creating these reports from raw clinical data is time-intensive and error-prone. Structured CDS documents ensure decisions are traceable, evidence-graded, and aligned with current guidelines."
targetAudience: "clinical researchers generating cohort analysis reports for trial design, pharmaceutical scientists building biomarker-stratified patient profiles, medical affairs teams creating evidence-based treatment recommendations, health outcomes researchers compiling guideline-concordance analyses, translational medicine teams bridging bench data to clinical decision-making"
exampleUseCase: "\"Generate a clinical decision support report for our oncology cohort\" → CDS package: biomarker-stratified patient cohort analysis with outcome correlations, treatment recommendation matrix aligned to NCCN guidelines, evidence quality grading for each recommendation, patient subgroup response predictions, and a clinical summary formatted for tumor board presentation"
...
tags: ["scientific", "claude-code", "clinical-decision-support", "bioinformatics", "chemistry", "statistics", "visualization", "database"]
...
```bash
mkdir -p ~/Projects/clinical-decision-support
```
...
```bash
mv ~/Downloads/CLAUDE.md ~/Projects/clinical-decision-support/
```

### Step 3: Start Claude Code

```bash
cd ~/Projects/clinical-decision-support
claude
```
