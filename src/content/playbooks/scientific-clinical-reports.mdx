---
title: "Scientific Clinical Reports"
...
seoHook: "Clinical reports have rigid formatting requirements — CARE guidelines for case reports, ICH-E3 for trial reports, SOAP for patient notes — and getting the structure wrong means rejection or compliance issues. Writing a clinical study report from scratch takes weeks when the template, structure, and required sections are well-defined."
targetAudience: "clinical research associates writing study reports for regulatory submissions, physicians drafting case reports for journal publication, medical writers preparing ICH-E3 compliant clinical study reports, residents creating structured patient documentation, pharmacovigilance teams documenting serious adverse events"
exampleUseCase: "\"Write a case report following CARE guidelines for our rare disease patient\" → Structured clinical report: CARE-compliant case report with patient presentation timeline, diagnostic workup summary, treatment course with outcomes, structured discussion with literature context, patient consent documentation framework, and properly formatted tables and figures"
...
tags: ["scientific", "claude-code", "clinical-reports", "chemistry", "statistics", "visualization", "database", "protein"]
...
```bash
mkdir -p ~/Projects/clinical-reports
```
...
```bash
mv ~/Downloads/CLAUDE.md ~/Projects/clinical-reports/
```

### Step 3: Start Claude Code

```bash
cd ~/Projects/clinical-reports
claude
```
...
- **Read the docs**: Check the official clinical-reports documentation for latest API changes
