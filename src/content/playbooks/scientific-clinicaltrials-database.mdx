---
title: "Scientific Clinicaltrials Database"
...
seoHook: "You need to find all Phase 2+ trials for your drug target, track competitor trial timelines, or identify patient recruitment sites — but manually searching ClinicalTrials.gov and exporting data is clunky. The v2 API lets you programmatically search by condition, drug, phase, status, or location and build competitive intelligence dashboards from trial data."
targetAudience: "clinical operations teams tracking competitive trial landscapes, pharmaceutical business development teams scouting partnership opportunities, clinical researchers identifying trials for meta-analysis or systematic review, patient advocacy groups matching patients to eligible trials, regulatory affairs teams monitoring approval-track trial progress"
exampleUseCase: "\"Find all active Phase 2-3 trials for our target indication with enrollment status\" → Trial landscape: filtered trial list with NCT IDs, sponsors, phases, and enrollment numbers, competitive timeline analysis showing expected readout dates, geographic distribution of recruitment sites, endpoint comparison across trials, and formatted export for competitive intelligence tracking"
...
tags: ["scientific", "claude-code", "clinicaltrials-database", "chemistry", "visualization", "database", "protein", "writing"]
...
```bash
mkdir -p ~/Projects/clinicaltrials-database
```
...
```bash
mv ~/Downloads/CLAUDE.md ~/Projects/clinicaltrials-database/
```

### Step 3: Start Claude Code

```bash
cd ~/Projects/clinicaltrials-database
claude
```
