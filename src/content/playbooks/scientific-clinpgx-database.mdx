---
title: "Scientific Clinpgx Database"
...
seoHook: "A patient's CYP2D6 genotype means the standard dose of codeine could be lethal — or completely ineffective. Pharmacogenomics data from ClinPGx tells you which gene variants affect which drugs, what the CPIC dosing guidelines recommend, and how to translate genotype into actionable prescribing decisions."
targetAudience: "clinical pharmacologists implementing pharmacogenomic-guided prescribing, precision medicine researchers studying gene-drug interactions, pharmacists interpreting pharmacogenomic test results, clinical geneticists advising on genotype-guided dosing, pharmaceutical researchers assessing pharmacogenomic implications for new drugs"
exampleUseCase: "\"Query gene-drug interactions for our panel of 10 psychiatric medications\" → Pharmacogenomics report: gene-drug interaction data for all 10 drugs, CPIC guideline-based dosing recommendations per genotype, allele function classifications, clinical actionability assessment for each interaction, and a formatted reference table for clinical implementation"
...
tags: ["scientific", "claude-code", "clinpgx-database", "bioinformatics", "chemistry", "database", "genomics", "writing"]
...
```bash
mkdir -p ~/Projects/clinpgx-database
```
...
```bash
mv ~/Downloads/CLAUDE.md ~/Projects/clinpgx-database/
```

### Step 3: Start Claude Code

```bash
cd ~/Projects/clinpgx-database
claude
```
...
- **Read the docs**: Check the official clinpgx-database documentation for latest API changes
