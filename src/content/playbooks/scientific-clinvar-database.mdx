---
title: "Scientific Clinvar Database"
...
seoHook: "Your whole-genome sequencing identified 50,000 variants and you need to know which ones are pathogenic. ClinVar is the gold standard for variant clinical significance — but programmatic access via E-utilities lets you batch-annotate VCFs, filter by pathogenicity, and build clinical-grade variant reports at scale."
targetAudience: "clinical geneticists interpreting patient variant reports, bioinformaticians building variant annotation pipelines, genetic counselors assessing variant pathogenicity for patients, genomic medicine researchers studying variant-disease associations, diagnostic labs automating ClinVar annotation in their sequencing workflows"
exampleUseCase: "\"Annotate our patient VCF with ClinVar pathogenicity classifications\" → Variant annotation: ClinVar lookup for all coding variants, pathogenicity classification with review status and evidence level, conflicting interpretation flagging, gene-disease association context, and a clinical-grade variant report sorted by significance"
...
tags: ["scientific", "claude-code", "clinvar-database", "database", "protein", "genomics", "writing"]
...
```bash
mkdir -p ~/Projects/clinvar-database
```
...
```bash
mv ~/Downloads/CLAUDE.md ~/Projects/clinvar-database/
```

### Step 3: Start Claude Code

```bash
cd ~/Projects/clinvar-database
claude
```
...
- **Read the docs**: Check the official clinvar-database documentation for latest API changes
