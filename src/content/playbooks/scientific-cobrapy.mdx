...
seoHook: "You want to predict how knocking out a gene will affect cell growth, or which metabolic reactions limit production of your target compound — but metabolic networks have thousands of reactions with complex interdependencies. COBRApy runs flux balance analysis, gene knockouts, and flux variability analysis on genome-scale metabolic models."
targetAudience: "metabolic engineers optimizing microbial production strains, systems biologists modeling cellular metabolism, synthetic biology researchers designing metabolic pathways, bioprocess engineers predicting growth phenotypes, graduate students learning constraint-based metabolic modeling"
exampleUseCase: "\"Predict the effect of gene knockouts on ethanol production in our yeast model\" → COBRA analysis: FBA growth rate predictions for wild-type, single and double gene knockout screen with growth and production phenotypes, flux variability analysis identifying flexible vs. constrained reactions, and essential gene predictions with production coupling analysis"
...
tags: ["scientific", "claude-code", "cobrapy", "chemistry", "visualization", "database", "protein", "genomics"]
...
```bash
mv ~/Downloads/CLAUDE.md ~/Projects/cobrapy/
```
