---
title: "Scientific Cosmic Database"
...
seoHook: "You need to know the mutation landscape of your oncology target — which mutations are driver vs. passenger, their frequency across cancer types, and the mutational signatures associated with them. COSMIC is the most comprehensive cancer mutation database, curating somatic mutations, gene fusions, and mutational signatures from published literature and large-scale screens."
targetAudience: "cancer researchers studying somatic mutation patterns across tumor types, oncology drug developers validating targets against mutation frequency data, translational scientists identifying patient stratification biomarkers, clinical genomics teams interpreting tumor sequencing results, precision oncology researchers matching patients to targeted therapies"
exampleUseCase: "\"Query COSMIC for the mutation landscape of EGFR across all cancer types\" → Mutation analysis: somatic mutation frequency by cancer type, hotspot mutation identification with functional impact, Cancer Gene Census classification, associated mutational signatures, gene fusion data, and a summary report suitable for target validation documentation"
...
tags: ["scientific", "claude-code", "cosmic-database", "bioinformatics", "chemistry", "database", "protein", "genomics"]
...
```bash
mkdir -p ~/Projects/cosmic-database
```
...
```bash
mv ~/Downloads/CLAUDE.md ~/Projects/cosmic-database/
```

### Step 3: Start Claude Code

```bash
cd ~/Projects/cosmic-database
claude
```
...
- **Read the docs**: Check the official cosmic-database documentation for latest API changes
