---
title: "Scientific Datacommons Client"
description: "Work with Data Commons, a platform providing programmatic access to public statistical data from global sources. Use this skill when working with demographic data, economic indicators, health statistics, environmental data, or any public datasets ..."
seoHook: "You need population demographics, GDP data, health statistics, and climate indicators — but each dataset lives on a different government website with a different format and API. Data Commons unifies public statistical data from the World Bank, CDC, Census Bureau, and dozens more into a single queryable knowledge graph."
targetAudience: "data journalists combining public datasets for investigative stories, policy researchers analyzing socioeconomic indicators across regions, social scientists studying demographic trends with public data, data analysts building dashboards from government statistics, students and educators working with real-world public datasets for coursework"
exampleUseCase: "\"Compare health outcomes and income levels across US counties\" → Data Commons analysis: county-level data retrieval for median income, life expectancy, and health insurance coverage, correlation analysis across indicators, geographic visualization-ready output, time series trends for the last 10 years, and a formatted dataset combining all indicators"
...
tags: ["scientific", "claude-code", "datacommons-client", "statistics", "visualization", "database", "finance"]
...
```bash
mkdir -p ~/Projects/datacommons-client
```
...
```bash
mv ~/Downloads/CLAUDE.md ~/Projects/datacommons-client/
```

### Step 3: Start Claude Code

```bash
cd ~/Projects/datacommons-client
claude
```
