---
title: "Scientific Deepchem"
description: "Molecular ML with diverse featurizers and pre-built datasets. Use for property prediction (ADMET, toxicity) with traditional ML or GNNs when you want extensive featurization options and MoleculeNet benchmarks. Best for quick experiments with pre-t..."
seoHook: "Predicting molecular properties like toxicity, solubility, or binding affinity requires featurizing molecules and training models — but building the pipeline from scratch means dealing with molecular representations, graph neural networks, and chemistry-specific data splits. DeepChem provides pre-built featurizers, datasets, and models for molecular ML out of the box."
targetAudience: "computational chemists building ADMET prediction models, drug discovery data scientists training property prediction models, ML researchers benchmarking on MoleculeNet datasets, pharmaceutical scientists predicting toxicity for compound screening, graduate students learning molecular machine learning"
exampleUseCase: "\"Build a solubility prediction model for our compound library\" → DeepChem pipeline: molecular featurization with graph convolution, model training on ESOL benchmark for transfer learning, fine-tuning on your compound data, performance evaluation with scaffold split, prediction for new compounds with uncertainty estimates, and comparison against baseline models"
...
tags: ["scientific", "claude-code", "deepchem", "bioinformatics", "machine-learning", "chemistry", "statistics", "visualization"]
...
```bash
mv ~/Downloads/CLAUDE.md ~/Projects/deepchem/
```

### Step 3: Start Claude Code

```bash
cd ~/Projects/deepchem
claude
```
...
- **Read the docs**: Check the official deepchem documentation for latest API changes
