...
seoHook: "Your ChIP-seq data is in BAM files and you need publication-quality heatmaps around TSS, sample correlation plots for QC, and normalized bigWig tracks for your genome browser. deepTools handles the entire NGS visualization pipeline — from BAM to heatmap — with commands designed specifically for epigenomics and transcriptomics data."
targetAudience: "epigenomics researchers creating publication-quality ChIP-seq and ATAC-seq figures, bioinformaticians building NGS QC and visualization pipelines, graduate students analyzing their first ChIP-seq or RNA-seq experiment, computational biologists generating normalized coverage tracks for genome browsers, core facility bioinformaticians processing NGS data for collaborators"
exampleUseCase: "\"Create QC plots and heatmaps for our ChIP-seq experiment\" → deepTools analysis: sample correlation matrix and PCA for QC, normalized bigWig conversion from BAM files, TSS-centered heatmap with profile plot, peak-centered signal enrichment visualization, and fingerprint plot for IP enrichment assessment"
...
tags: ["scientific", "claude-code", "deeptools", "bioinformatics", "visualization", "database", "genomics"]
