---
title: "Scientific Dnanexus Integration"
...
seoHook: "Your genomics pipeline runs locally but can't scale to 10,000 whole genomes. DNAnexus provides cloud infrastructure for genomic analysis — but building apps, managing data uploads, and orchestrating workflows through the dxpy SDK requires knowing the platform's patterns. Proper integration lets you run FASTQ-to-VCF pipelines at population scale."
targetAudience: "bioinformaticians deploying genomics pipelines on DNAnexus cloud, genomics core facilities processing high-throughput sequencing data, clinical genomics teams running validated analysis pipelines, research institutions migrating local compute to cloud genomics, pipeline developers building portable genomics workflows"
exampleUseCase: "\"Build and deploy our variant calling pipeline on DNAnexus\" → DNAnexus integration: dxpy-based app/applet development for each pipeline step, data upload automation for FASTQ files, workflow orchestration connecting alignment through variant calling, output management with structured project organization, and execution monitoring with error handling"
...
tags: ["scientific", "claude-code", "dnanexus-integration", "bioinformatics", "visualization", "database", "protein", "genomics"]
...
```bash
mkdir -p ~/Projects/dnanexus-integration
```
...
```bash
mv ~/Downloads/CLAUDE.md ~/Projects/dnanexus-integration/
```

### Step 3: Start Claude Code

```bash
cd ~/Projects/dnanexus-integration
claude
```
