---
title: "Scientific Drugbank Database"
description: "Access and analyze comprehensive drug information from the DrugBank database including drug properties, interactions, targets, pathways, chemical structures, and pharmacology data. This skill should be used when working with pharmaceutical data, d..."
seoHook: "You need comprehensive drug data — targets, interactions, pathways, pharmacokinetics, and chemical properties — but it's scattered across FDA labels, publications, and databases. DrugBank consolidates it all for 15,000+ drugs into a structured, queryable database that's the gold standard for pharmaceutical research and drug repurposing studies."
targetAudience: "pharmaceutical researchers investigating drug-target interactions, pharmacologists studying drug mechanisms and pathways, drug repurposing scientists screening existing drugs for new indications, clinical pharmacists checking drug interaction profiles, medicinal chemistry students learning drug properties and pharmacology"
exampleUseCase: "\"Query DrugBank for all approved drugs targeting our kinase with interaction and pathway data\" → Drug intelligence report: approved and investigational drugs for the target, mechanism of action details, drug-drug interaction profiles, metabolic pathway involvement, chemical structure data, and pharmacokinetic properties comparison across the drug class"
...
tags: ["scientific", "claude-code", "drugbank-database", "bioinformatics", "chemistry", "visualization", "database", "protein"]
...
```bash
mkdir -p ~/Projects/drugbank-database
```
...
```bash
mv ~/Downloads/CLAUDE.md ~/Projects/drugbank-database/
```

### Step 3: Start Claude Code

```bash
cd ~/Projects/drugbank-database
claude
```
