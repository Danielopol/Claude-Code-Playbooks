---
title: "Scientific Ensembl Database"
...
seoHook: "You need gene coordinates, transcript isoforms, variant annotations, and ortholog mappings — and Ensembl has it all for 250+ species. But the REST API has hundreds of endpoints and navigating them without guidance means hours of trial and error. Programmatic Ensembl access puts the world's most comprehensive genome annotation at your fingertips."
targetAudience: "genomics researchers retrieving gene annotations and sequences across species, bioinformaticians building variant annotation pipelines with VEP predictions, comparative genomics teams querying ortholog relationships, clinical geneticists annotating variants with Ensembl data, evolutionary biologists studying gene conservation across taxa"
exampleUseCase: "\"Query Ensembl for gene annotations, variants, and orthologs for our candidate gene list\" → Ensembl data package: gene coordinates and transcript isoforms for each candidate, known variants with VEP consequence predictions, ortholog mappings across key model organisms, regulatory region annotations, and a consolidated gene information table"
...
tags: ["scientific", "claude-code", "ensembl-database", "bioinformatics", "database", "protein", "genomics"]
...
```bash
mkdir -p ~/Projects/ensembl-database
```
...
```bash
mv ~/Downloads/CLAUDE.md ~/Projects/ensembl-database/
```

### Step 3: Start Claude Code

```bash
cd ~/Projects/ensembl-database
claude
```
...
- **Read the docs**: Check the official ensembl-database documentation for latest API changes
