...
seoHook: "Your phylogenetic tree has 500 taxa and you need to prune clades, detect duplications, map NCBI taxonomy, and render a publication-quality figure — but matplotlib can't handle tree layouts. ETE Toolkit does tree manipulation, evolutionary event detection, and visualization in Python, purpose-built for phylogenomics workflows."
targetAudience: "evolutionary biologists building and analyzing phylogenetic trees, bioinformaticians integrating phylogenetics into analysis pipelines, comparative genomics researchers studying orthology and paralogy, microbiology teams constructing pathogen phylogenies, graduate students creating publication-quality tree visualizations"
exampleUseCase: "\"Analyze our phylogenetic tree of 200 bacterial genomes\" → ETE analysis: tree loading from Newick format, clade pruning and rerooting, duplication/speciation event detection, NCBI taxonomy annotation, ortholog/paralog classification, and publication-quality tree rendering as PDF/SVG with custom annotations"
...
tags: ["scientific", "claude-code", "etetoolkit", "visualization", "database", "protein", "genomics", "writing"]
...
```bash
mv ~/Downloads/CLAUDE.md ~/Projects/etetoolkit/
```

### Step 3: Start Claude Code

```bash
cd ~/Projects/etetoolkit
claude
```
