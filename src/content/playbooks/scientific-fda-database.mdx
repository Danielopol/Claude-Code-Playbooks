...
seoHook: "You need adverse event reports for a drug class, 510(k) clearance data for competitor devices, or recall history for a product category — but the FDA website search is clunky and exports are limited. The openFDA API gives you programmatic access to drug safety data, device submissions, and regulatory actions at scale."
targetAudience: "pharmacovigilance scientists analyzing adverse event trends, medical device regulatory teams tracking 510(k) and PMA submissions, drug safety researchers studying post-market surveillance data, regulatory affairs professionals monitoring FDA actions in their therapeutic area, pharmaceutical competitive intelligence teams tracking competitor approvals"
exampleUseCase: "\"Query FDA adverse events for our drug class over the last 5 years\" → FDA data analysis: adverse event report counts by reaction type, serious vs. non-serious event trends over time, demographic breakdown of reported cases, signal detection for emerging safety concerns, comparison across drugs in the class, and a formatted safety summary report"
...
tags: ["scientific", "claude-code", "fda-database", "chemistry", "database", "protein", "genomics", "finance"]
...
```bash
mv ~/Downloads/CLAUDE.md ~/Projects/fda-database/
```

### Step 3: Start Claude Code

```bash
cd ~/Projects/fda-database
claude
```
