---
title: "Scientific Geniml"
description: "This skill should be used when working with genomic interval data (BED files) for machine learning tasks. Use for training region embeddings (Region2Vec, BEDspace), single-cell ATAC-seq analysis (scEmbed), building consensus peaks (universes), or ..."
seoHook: "Genomic intervals (peaks, regulatory regions, binding sites) are stored as BED files, but standard ML tools don't understand genomic coordinates. GENIML provides region embeddings, interval-aware ML, and consensus peak building — bridging the gap between genomic interval data and machine learning workflows."
targetAudience: "epigenomics researchers applying ML to ChIP-seq and ATAC-seq peak data, single-cell biologists analyzing scATAC-seq with embedding methods, computational biologists building region-level classifiers, bioinformaticians creating genomic interval universes from multiple experiments, graduate students exploring ML applications in epigenomics"
exampleUseCase: "\"Build region embeddings from our ATAC-seq peak sets for cell type classification\" → GENIML pipeline: consensus universe construction from multiple BED files, Region2Vec embedding training on genomic intervals, scATAC-seq cell embedding with scEmbed, cell type clustering and visualization, and classification model evaluation with cross-validation"
...
tags: ["scientific", "claude-code", "geniml", "machine-learning", "statistics", "visualization", "database", "genomics"]
...
```bash
mv ~/Downloads/CLAUDE.md ~/Projects/geniml/
```
