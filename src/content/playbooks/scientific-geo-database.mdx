...
seoHook: "A paper reports a key gene expression finding but doesn't share processed data — just a GEO accession number. Downloading the right GSE dataset, parsing SOFT/Matrix files, and extracting the expression values you need requires navigating GEO's structure. Programmatic access lets you search, download, and parse GEO data directly into your analysis."
targetAudience: "bioinformaticians downloading and reanalyzing public gene expression datasets, transcriptomics researchers building meta-analyses across multiple GEO studies, graduate students accessing microarray and RNA-seq data for their projects, computational biologists benchmarking methods on public expression datasets, lab scientists retrieving expression data to validate their own results"
exampleUseCase: "\"Download and parse gene expression data from 5 GEO datasets for our meta-analysis\" → GEO data retrieval: dataset search and metadata extraction, SOFT/Matrix file parsing for expression values, sample annotation mapping, cross-dataset gene ID harmonization, and a combined expression matrix ready for differential expression or meta-analysis"
...
tags: ["scientific", "claude-code", "geo-database", "bioinformatics", "chemistry", "statistics", "visualization", "database"]
...
```bash
mv ~/Downloads/CLAUDE.md ~/Projects/geo-database/
```

### Step 3: Start Claude Code

```bash
cd ~/Projects/geo-database
claude
```
...
- **Read the docs**: Check the official geo-database documentation for latest API changes
