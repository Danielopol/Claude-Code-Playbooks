---
title: "Scientific Geopandas"
description: "Python library for working with geospatial vector data including shapefiles, GeoJSON, and GeoPackage files. Use when working with geographic data for spatial analysis, geometric operations, coordinate transformations, spatial joins, overlay operat..."
seoHook: "You have shapefiles, GeoJSON, and GeoPackage files from different sources that need spatial joins, coordinate transformations, and overlay operations. GeoPandas extends pandas with geometry support so you can manipulate geospatial vector data as naturally as a regular DataFrame — no desktop GIS required."
targetAudience: "GIS analysts performing spatial analysis in Python instead of desktop software, urban planners working with parcel and zoning shapefiles, environmental scientists overlaying field data with administrative boundaries, data scientists adding spatial dimensions to tabular datasets, transportation engineers analyzing road networks and service areas"
exampleUseCase: "\"Join our field sampling points to census tract boundaries and compute statistics per tract\" → GeoPandas workflow: shapefile and GeoJSON loading with CRS alignment, spatial join of points to polygons, per-tract aggregation statistics, choropleth map visualization, and export to GeoPackage for sharing with GIS colleagues"
...
tags: ["scientific", "claude-code", "geopandas", "visualization", "database", "protein", "writing"]
createdAt: "2026-03-06"
---

## What This Does

A scientific skill for geopandas workflows with Claude Code.
...
```bash
mv ~/Downloads/CLAUDE.md ~/Projects/geopandas/
```

### Step 3: Start Claude Code

```bash
cd ~/Projects/geopandas
claude
```
