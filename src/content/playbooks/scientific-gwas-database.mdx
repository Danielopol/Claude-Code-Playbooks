---
title: "Scientific Gwas Database"
...
seoHook: "You have a list of SNPs from your genotyping study and need to know which traits they've been associated with in published GWAS. The NHGRI-EBI GWAS Catalog aggregates thousands of studies into a single searchable database of SNP-trait associations with p-values and effect sizes."
targetAudience: "genetic epidemiologists looking up known SNP-trait associations for their study variants, statistical geneticists building polygenic risk scores from published GWAS summary statistics, clinical geneticists checking if patient variants have known disease associations, bioinformaticians annotating variant lists with GWAS catalog data, human genetics researchers exploring the genetic architecture of complex traits"
exampleUseCase: "\"Annotate our GWAS hits with known trait associations from the GWAS Catalog\" → GWAS Catalog query: variant lookup by rs ID with trait associations and p-values, gene-based search for all associated SNPs, disease/trait filtering across studies, summary statistics retrieval for polygenic scoring, and an annotated variant table with published association evidence"
...
tags: ["scientific", "claude-code", "gwas-database", "bioinformatics", "statistics", "visualization", "database", "protein"]
...
```bash
mkdir -p ~/Projects/gwas-database
```
...
```bash
mv ~/Downloads/CLAUDE.md ~/Projects/gwas-database/
```

### Step 3: Start Claude Code

```bash
cd ~/Projects/gwas-database
claude
```
