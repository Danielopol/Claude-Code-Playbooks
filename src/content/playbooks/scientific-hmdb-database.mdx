---
title: "Scientific Hmdb Database"
...
seoHook: "Your mass spec run identified 50 metabolite features but you need chemical properties, biological roles, and pathway context for each. HMDB contains 220,000+ metabolite entries with NMR/MS spectra, biomarker data, and pathway annotations — the most comprehensive human metabolomics reference for compound identification."
targetAudience: "metabolomics researchers identifying compounds from mass spectrometry and NMR data, clinical biochemists looking up metabolite biomarker associations for diagnostic studies, systems biologists mapping metabolites to biological pathways, analytical chemists retrieving reference spectra for compound identification, nutritional scientists studying metabolite profiles in health and disease"
exampleUseCase: "\"Identify and annotate the metabolites from our untargeted LC-MS experiment\" → HMDB query: metabolite search by exact mass and molecular formula, chemical property retrieval including structures and classification, reference NMR and MS spectra for confirmation, biological role and biomarker disease associations, and pathway mapping for metabolic context"
...
tags: ["scientific", "claude-code", "hmdb-database", "chemistry", "visualization", "database", "protein", "genomics"]
...
```bash
mkdir -p ~/Projects/hmdb-database
```
...
```bash
mv ~/Downloads/CLAUDE.md ~/Projects/hmdb-database/
```

### Step 3: Start Claude Code

```bash
cd ~/Projects/hmdb-database
claude
```
