---
title: "Scientific Hypogenic"
description: "Automated LLM-driven hypothesis generation and testing on tabular datasets. Use when you want to systematically explore hypotheses about patterns in empirical data (e.g., deception detection, content analysis). Combines literature insights with da..."
seoHook: "You have a dataset with dozens of features and no clear hypotheses about what drives your outcome variable. HypoGenic uses LLMs to systematically generate, test, and refine hypotheses against your data — automating the exploratory analysis that normally takes days of manual investigation."
targetAudience: "social scientists exploring patterns in survey and behavioral datasets, data scientists performing automated exploratory analysis on tabular data, NLP researchers testing hypotheses about content features and outcomes, behavioral researchers studying deception detection and content analysis patterns, quantitative researchers who want systematic hypothesis exploration before confirmatory analysis"
exampleUseCase: "\"Explore what features predict deception in our interview transcript dataset\" → HypoGenic pipeline: automated hypothesis generation from data features and literature, statistical testing of each hypothesis against the dataset, iterative refinement based on test results, ranked list of supported hypotheses with effect sizes, and a summary report of discovered patterns with confidence levels"
...
tags: ["scientific", "claude-code", "hypogenic", "statistics", "visualization", "database", "protein", "genomics"]
...
```bash
mv ~/Downloads/CLAUDE.md ~/Projects/hypogenic/
```

### Step 3: Start Claude Code

```bash
cd ~/Projects/hypogenic
claude
```
...
- **Read the docs**: Check the official hypogenic documentation for latest API changes
