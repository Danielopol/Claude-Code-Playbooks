...
seoHook: "You have intriguing experimental observations but struggle to formulate them into rigorous, testable hypotheses with clear predictions and proposed mechanisms. Structured hypothesis generation frameworks help you go from 'interesting pattern' to 'publishable research question' with proper experimental design."
targetAudience: "graduate students learning to formulate testable hypotheses from experimental observations, early-career researchers developing research proposals and grant applications, principal investigators brainstorming new research directions from preliminary data, interdisciplinary scientists translating observations across fields into testable frameworks, science educators teaching the scientific method and hypothesis formulation"
exampleUseCase: "\"Formulate testable hypotheses from our preliminary data showing unexpected protein localization\" → Hypothesis framework: structured hypothesis statements with null and alternative forms, predicted outcomes for each hypothesis, proposed mechanisms with supporting literature, experimental designs to test each prediction, and a prioritized research plan with feasibility assessments"
...
tags: ["scientific", "claude-code", "hypothesis-generation", "statistics", "visualization", "database", "protein", "genomics"]
...
```bash
mkdir -p ~/Projects/hypothesis-generation
```
...
```bash
mv ~/Downloads/CLAUDE.md ~/Projects/hypothesis-generation/
```

### Step 3: Start Claude Code

```bash
cd ~/Projects/hypothesis-generation
claude
```
...
- **Read the docs**: Check the official hypothesis-generation documentation for latest API changes
