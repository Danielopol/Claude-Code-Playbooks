---
title: "Scientific Kegg Database"
description: "Direct REST API access to KEGG (academic use only). Pathway analysis, gene-pathway mapping, metabolic pathways, drug interactions, ID conversion. For Python workflows with multiple databases, prefer bioservices. Use this for direct HTTP/REST work ..."
seoHook: "Your gene list from a differential expression experiment needs pathway context — which metabolic and signaling pathways are enriched? KEGG maps genes to curated biological pathways, metabolic networks, and drug interaction data, providing the biological context that turns a gene list into mechanistic insight."
targetAudience: "systems biologists mapping gene lists to metabolic and signaling pathways, bioinformaticians running pathway enrichment analysis on expression data, pharmacologists studying drug-target-pathway interactions, metabolic engineers analyzing metabolic network routes, computational biologists converting gene IDs across databases using KEGG cross-references"
exampleUseCase: "\"Map our differentially expressed genes to KEGG pathways and identify enriched metabolic routes\" → KEGG workflow: gene-to-pathway mapping via REST API, pathway enrichment analysis with statistical testing, metabolic network visualization with highlighted genes, drug interaction lookup for therapeutic targets, and a pathway report with colored KEGG maps showing affected routes"
...
tags: ["scientific", "claude-code", "kegg-database", "bioinformatics", "chemistry", "visualization", "database", "protein"]
