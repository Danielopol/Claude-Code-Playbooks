---
title: "Scientific Labarchive Integration"
...
seoHook: "Your lab notebook entries are disconnected from your computational analysis — results in one place, protocols in another, raw data somewhere else. LabArchives API integration lets you programmatically create entries, attach analysis outputs, and sync with Jupyter notebooks, keeping your electronic lab records and computational work in one place."
targetAudience: "lab managers enforcing electronic lab notebook compliance across research groups, graduate students integrating computational analysis outputs with experimental records, principal investigators building automated data backup workflows for lab notebooks, research compliance officers ensuring reproducibility and audit trails, core facility managers connecting instrument outputs to LabArchives entries"
exampleUseCase: "\"Automatically sync our Jupyter analysis results into LabArchives notebook entries\" → ELN integration: API-based notebook access and entry creation, automated attachment of analysis outputs and figures, Protocols.io protocol linking to experimental entries, Jupyter notebook export and attachment workflow, and a backup script ensuring all lab data is captured in the ELN"
...
tags: ["scientific", "claude-code", "labarchive-integration", "bioinformatics", "visualization", "database", "protein", "genomics"]
...
```bash
mkdir -p ~/Projects/labarchive-integration
```
...
```bash
mv ~/Downloads/CLAUDE.md ~/Projects/labarchive-integration/
```

### Step 3: Start Claude Code

```bash
cd ~/Projects/labarchive-integration
claude
```
