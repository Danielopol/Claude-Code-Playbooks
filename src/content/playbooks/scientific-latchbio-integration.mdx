---
title: "Scientific Latchbio Integration"
...
seoHook: "Your bioinformatics pipeline runs locally but your collaborators can't reproduce it without your exact environment setup. Latch lets you wrap Python pipelines with simple decorators and deploy them as serverless cloud workflows — shareable, scalable, and reproducible without any DevOps."
targetAudience: "bioinformaticians deploying analysis pipelines to the cloud without infrastructure management, computational biologists sharing reproducible workflows with wet lab collaborators, genomics core facilities offering self-service pipeline execution to researchers, pipeline developers migrating Nextflow or Snakemake workflows to a managed platform, startup scientists needing scalable bioinformatics without building infrastructure"
exampleUseCase: "\"Deploy our RNA-seq pipeline to Latch so the wet lab team can run it themselves\" → Latch deployment: pipeline wrapping with @workflow and @task decorators, LatchFile/LatchDir parameter configuration for cloud storage, serverless deployment with automatic scaling, web UI generation for non-computational users, and workflow versioning for reproducible analysis across the team"
...
tags: ["scientific", "claude-code", "latchbio-integration", "bioinformatics", "visualization", "database", "protein", "genomics"]
...
```bash
mkdir -p ~/Projects/latchbio-integration
```
...
```bash
mv ~/Downloads/CLAUDE.md ~/Projects/latchbio-integration/
```

### Step 3: Start Claude Code

```bash
cd ~/Projects/latchbio-integration
claude
```
