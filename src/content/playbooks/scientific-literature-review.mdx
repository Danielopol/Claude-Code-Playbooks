...
seoHook: "A thorough literature review means searching PubMed, arXiv, bioRxiv, and Semantic Scholar separately, deduplicating results, and synthesizing hundreds of abstracts — a process that takes weeks. Systematic review automation searches multiple databases simultaneously, screens results, and produces structured syntheses following PRISMA guidelines."
targetAudience: "PhD students conducting systematic reviews for their dissertation background chapters, clinical researchers performing systematic reviews and meta-analyses for evidence synthesis, grant writers surveying the literature landscape to position their research proposals, review article authors synthesizing findings across hundreds of papers, research teams establishing the state of the art before starting new projects"
exampleUseCase: "\"Conduct a systematic review on CRISPR delivery methods published in the last 3 years\" → Literature review pipeline: multi-database search across PubMed, bioRxiv, and Semantic Scholar, duplicate removal and relevance screening, structured data extraction from included studies, thematic synthesis of delivery approaches and outcomes, and a PRISMA-formatted review document with citation management"
...
tags: ["scientific", "claude-code", "literature-review", "bioinformatics", "machine-learning", "chemistry", "statistics", "visualization"]
...
```bash
mkdir -p ~/Projects/literature-review
```
...
```bash
mv ~/Downloads/CLAUDE.md ~/Projects/literature-review/
```

### Step 3: Start Claude Code

```bash
cd ~/Projects/literature-review
claude
```
