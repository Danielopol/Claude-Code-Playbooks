---
title: "Scientific Matchms"
description: "Spectral similarity and compound identification for metabolomics. Use for comparing mass spectra, computing similarity scores (cosine, modified cosine), and identifying unknown compounds from spectral libraries. Best for metabolite identification,..."
seoHook: "Your mass spectrometry experiment produced hundreds of unknown spectra that need to be matched against reference libraries for compound identification. matchms computes spectral similarity scores using cosine, modified cosine, and other metrics — turning raw spectra into identified metabolites with confidence scores."
targetAudience: "metabolomics researchers identifying unknown compounds by matching experimental spectra to libraries, analytical chemists computing spectral similarity for quality control and compound verification, natural products chemists discovering novel metabolites through spectral networking, environmental scientists identifying pollutants from mass spectrometry data, pharmaceutical scientists screening compound libraries using spectral matching"
exampleUseCase: "\"Match our experimental MS/MS spectra against GNPS library to identify metabolites\" → matchms pipeline: spectral data import and preprocessing with noise filtering, reference library loading from GNPS or MassBank, cosine and modified cosine similarity scoring, molecular networking from spectral similarities, and an identification report with matched compounds and confidence scores"
...
tags: ["scientific", "claude-code", "matchms", "chemistry", "visualization", "database", "protein", "genomics"]
...
```bash
mv ~/Downloads/CLAUDE.md ~/Projects/matchms/
```
...
- **Read the docs**: Check the official matchms documentation for latest API changes
