...
seoHook: "Your virtual screen returned 10,000 hits but most will fail in the clinic due to poor drug-likeness, PAINS interference, or toxicity flags. Medicinal chemistry filters apply Lipinski, Veber, PAINS, and structural alert rules in one pass — cutting your compound list to drug-like candidates before you waste synthesis resources."
targetAudience: "medicinal chemists filtering virtual screening hits for drug-likeness before synthesis, computational chemists applying ADMET filters to compound libraries, pharmaceutical scientists prioritizing lead compounds using multi-parameter optimization, drug discovery teams triaging compound collections with structural alerts, cheminformatics engineers building automated compound filtering pipelines"
exampleUseCase: "\"Filter our 5,000 virtual screening hits for drug-likeness and remove PAINS compounds\" → MedChem filtering: Lipinski Rule of Five and Veber rule application, PAINS substructure filter to remove assay interference compounds, structural alert flagging for known toxicophores, molecular complexity and synthetic accessibility scoring, and a prioritized compound table with pass/fail annotations for each filter"
...
tags: ["scientific", "claude-code", "medchem", "chemistry", "database", "protein", "genomics", "finance"]
...
```bash
mv ~/Downloads/CLAUDE.md ~/Projects/medchem/
```
