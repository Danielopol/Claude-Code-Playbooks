---
title: "Scientific Metabolomics Workbench Database"
...
seoHook: "You need reference metabolomics data for your disease study but published datasets are scattered across supplementary files and institutional repositories. Metabolomics Workbench provides standardized REST API access to 4,200+ studies with MS/NMR data, RefMet nomenclature, and m/z search — the NIH's central metabolomics data repository."
targetAudience: "metabolomics researchers accessing public datasets for method benchmarking and meta-analysis, clinical scientists searching for metabolite biomarkers across disease studies, analytical chemists performing m/z lookups to identify unknown spectral features, biomarker discovery teams comparing metabolite profiles across published studies, bioinformaticians building metabolomics data integration pipelines"
exampleUseCase: "\"Search Metabolomics Workbench for diabetes metabolite profiles and download the raw data\" → Workbench query: study metadata search by disease and analytical platform, metabolite lookup with RefMet standardized nomenclature, m/z-based compound identification from spectral data, raw data download for reanalysis, and a summary table of metabolite changes across matched studies"
...
tags: ["scientific", "claude-code", "metabolomics-workbench-database", "chemistry", "visualization", "database", "protein", "genomics"]
...
```bash
mkdir -p ~/Projects/metabolomics-workbench-database
```
...
```bash
mv ~/Downloads/CLAUDE.md ~/Projects/metabolomics-workbench-database/
```

### Step 3: Start Claude Code

```bash
cd ~/Projects/metabolomics-workbench-database
claude
```
