---
title: "Scientific Molecular Dynamics"
...
seoHook: "Setting up a molecular dynamics simulation requires system preparation, force field selection, minimization, equilibration, and production runs — each with dozens of parameters that can silently produce garbage results. OpenMM and MDAnalysis workflows handle the full simulation pipeline from PDB to publication-ready trajectory analysis."
targetAudience: "computational chemists running molecular dynamics simulations of protein-ligand systems, structural biologists studying protein conformational dynamics and flexibility, drug discovery scientists computing binding free energies from MD trajectories, biophysicists analyzing protein folding and stability through simulation, graduate students learning molecular dynamics setup and trajectory analysis"
exampleUseCase: "\"Set up and run an MD simulation of our protein-drug complex and analyze the binding dynamics\" → MD pipeline: system preparation with force field assignment and solvation, energy minimization and NPT equilibration, production MD run with OpenMM, trajectory analysis with RMSD, RMSF, and contact maps via MDAnalysis, and free energy surface visualization of binding conformations"
category: "academic-research"
difficulty: "advanced"
timeToSetup: "15 minutes"
author: "K-Dense AI"
sourceUrl: "https://github.com/K-Dense-AI/claude-scientific-skills/tree/main/scientific-skills/molecular-dynamics"
tags: ["scientific", "claude-code", "molecular-dynamics", "chemistry", "visualization", "database", "protein", "genomics"]
...
```bash
mkdir -p ~/Projects/molecular-dynamics
```
...
```bash
mv ~/Downloads/CLAUDE.md ~/Projects/molecular-dynamics/
```

### Step 3: Start Claude Code

```bash
cd ~/Projects/molecular-dynamics
claude
```
