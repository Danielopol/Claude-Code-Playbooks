---
title: "Scientific Monarch Database"
...
seoHook: "A rare disease patient presents with an unusual phenotype combination and you need to find candidate genes — but relevant data is scattered across OMIM, ClinVar, HPO, and model organism databases. Monarch Initiative integrates all of these into a single knowledge graph for cross-species disease-gene-phenotype discovery."
targetAudience: "clinical geneticists using phenotype-to-gene mapping for rare disease diagnosis, rare disease researchers discovering candidate genes through cross-species phenotype comparison, genetic counselors investigating gene-disease associations across multiple databases, model organism researchers translating animal phenotypes to human disease relevance, bioinformaticians building disease gene prioritization pipelines"
exampleUseCase: "\"Find candidate genes for our patient's rare phenotype combination using Monarch\" → Monarch query: HPO-based phenotype search across the knowledge graph, disease-gene association retrieval from OMIM and Orphanet, cross-species phenotype matching from mouse and zebrafish models, gene prioritization by phenotype overlap score, and a candidate gene report with supporting evidence from each integrated database"
category: "academic-research"
difficulty: "beginner"
timeToSetup: "5 minutes"
author: "K-Dense AI"
sourceUrl: "https://github.com/K-Dense-AI/claude-scientific-skills/tree/main/scientific-skills/monarch-database"
tags: ["scientific", "claude-code", "monarch-database", "database", "genomics", "writing"]
...
```bash
mkdir -p ~/Projects/monarch-database
```
...
```bash
mv ~/Downloads/CLAUDE.md ~/Projects/monarch-database/
```

### Step 3: Start Claude Code

```bash
cd ~/Projects/monarch-database
claude
```
