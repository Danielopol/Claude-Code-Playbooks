---
title: "Scientific Opentargets Database"
...
seoHook: "Your drug discovery program needs to validate a target — is it genetically linked to the disease, is it tractable, what safety risks exist, and are competitors already pursuing it? Open Targets integrates genetics, genomics, and drug data into a single platform for systematic target identification and validation."
targetAudience: "drug discovery scientists validating therapeutic targets with integrated genetic and clinical evidence, pharmaceutical researchers assessing target tractability and safety profiles, computational biologists building target prioritization pipelines from multi-omics data, translational researchers linking GWAS findings to druggable targets, biotech analysts conducting competitive intelligence on drug target landscapes"
exampleUseCase: "\"Evaluate our candidate drug targets for Alzheimer's disease using Open Targets evidence\" → Open Targets query: target-disease association scores with evidence breakdown, genetic evidence from GWAS and rare disease studies, tractability assessment for small molecules and antibodies, known drug pipeline for each target, and a target prioritization matrix ranked by overall evidence strength"
category: "academic-research"
difficulty: "beginner"
timeToSetup: "5 minutes"
author: "K-Dense AI"
sourceUrl: "https://github.com/K-Dense-AI/claude-scientific-skills/tree/main/scientific-skills/opentargets-database"
tags: ["scientific", "claude-code", "opentargets-database", "bioinformatics", "chemistry", "visualization", "database", "genomics"]
...
```bash
mkdir -p ~/Projects/opentargets-database
```
...
```bash
mv ~/Downloads/CLAUDE.md ~/Projects/opentargets-database/
```

### Step 3: Start Claude Code

```bash
cd ~/Projects/opentargets-database
claude
```
