---
title: "Scientific Opentrons Integration"
description: "Official Opentrons Protocol API for OT-2 and Flex robots. Use when writing protocols specifically for Opentrons hardware with full access to Protocol API v2 features. Best for production Opentrons protocols, official API compatibility. For multi-v..."
seoHook: "Writing Opentrons protocols from scratch means hours debugging pipetting logic, labware definitions, and tip management — with mistakes costing reagents and time. The Protocol API v2 workflow guides you through writing correct, efficient protocols for OT-2 and Flex robots with proper error handling and deck layout."
targetAudience: "lab automation engineers writing liquid handling protocols for Opentrons robots, molecular biologists automating repetitive pipetting workflows like PCR setup and serial dilutions, genomics core facilities programming sample prep protocols for library construction, pharmaceutical scientists automating compound screening and plate reformatting, research labs transitioning manual protocols to automated Opentrons execution"
exampleUseCase: "\"Write an Opentrons OT-2 protocol for our 96-well PCR setup with serial dilutions\" → Opentrons protocol: deck layout configuration with labware and tip rack placement, pipetting logic for serial dilution series with proper tip management, PCR master mix distribution and template addition, protocol simulation for validation before running, and documented Python protocol ready for upload to the OT-2"
category: "academic-research"
difficulty: "intermediate"
timeToSetup: "10 minutes"
author: "K-Dense AI"
sourceUrl: "https://github.com/K-Dense-AI/claude-scientific-skills/tree/main/scientific-skills/opentrons-integration"
tags: ["scientific", "claude-code", "opentrons-integration", "database", "protein", "writing"]
...
```bash
mkdir -p ~/Projects/opentrons-integration
```
...
```bash
mv ~/Downloads/CLAUDE.md ~/Projects/opentrons-integration/
```

### Step 3: Start Claude Code

```bash
cd ~/Projects/opentrons-integration
claude
```
