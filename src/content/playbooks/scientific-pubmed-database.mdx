---
title: "Scientific Pubmed Database"
...
seoHook: "Your systematic review needs 2,000 PubMed results with MeSH-filtered Boolean queries, but the web interface limits bulk access and manual downloading is impractical. Direct PubMed REST API access via E-utilities enables advanced Boolean and MeSH queries, batch abstract retrieval, and citation management at scale."
targetAudience: "systematic review authors running complex Boolean PubMed queries for comprehensive literature searches, bioinformaticians building automated literature monitoring pipelines, clinical researchers retrieving evidence for clinical guidelines and meta-analyses, librarians performing expert-level database searches for research teams, science writers gathering published evidence on specific topics"
exampleUseCase: "\"Run a systematic PubMed search for our meta-analysis with MeSH terms and date filters\" → PubMed workflow: advanced Boolean query construction with MeSH terms and field tags, batch retrieval of matching article metadata and abstracts, citation export in standard formats for reference managers, search result deduplication and relevance scoring, and a PRISMA-compatible search documentation report"
category: "academic-research"
difficulty: "beginner"
timeToSetup: "5 minutes"
author: "K-Dense AI"
sourceUrl: "https://github.com/K-Dense-AI/claude-scientific-skills/tree/main/scientific-skills/pubmed-database"
tags: ["scientific", "claude-code", "pubmed-database", "chemistry", "database", "protein", "writing"]
...
```bash
mkdir -p ~/Projects/pubmed-database
```
...
```bash
mv ~/Downloads/CLAUDE.md ~/Projects/pubmed-database/
```

### Step 3: Start Claude Code

```bash
cd ~/Projects/pubmed-database
claude
```
