---
title: "Scientific Pydicom"
description: "Python library for working with DICOM (Digital Imaging and Communications in Medicine) files. Use this skill when reading, writing, or modifying medical imaging data in DICOM format, extracting pixel data from medical images (CT, MRI, X-ray, ultra..."
seoHook: "Your medical imaging pipeline needs to read DICOM headers, extract pixel arrays, anonymize patient data, or convert between formats — but DICOM's complex nested tag structure makes manual parsing a nightmare. PyDICOM reads, writes, and modifies DICOM files in Python with intuitive access to metadata, pixel data, and all standard DICOM tags."
targetAudience: "medical imaging researchers extracting pixel data and metadata from CT, MRI, and X-ray DICOM files, clinical informatics teams building DICOM anonymization pipelines for research data sharing, radiology AI developers preprocessing DICOM images for deep learning model training, medical physicists analyzing DICOM dose reports and imaging parameters, PACS administrators writing scripts for DICOM file management and routing"
exampleUseCase: "\"Anonymize our DICOM dataset and extract CT pixel arrays for ML training\" → PyDICOM pipeline: DICOM file reading with metadata and pixel data extraction, patient information anonymization following HIPAA de-identification rules, windowing and normalization of CT Hounsfield units, batch processing across study series and patients, and NumPy array output ready for medical image analysis"
category: "academic-research"
difficulty: "intermediate"
timeToSetup: "10 minutes"
author: "K-Dense AI"
sourceUrl: "https://github.com/K-Dense-AI/claude-scientific-skills/tree/main/scientific-skills/pydicom"
tags: ["scientific", "claude-code", "pydicom", "visualization", "database", "protein", "genomics", "writing"]
...
```bash
mv ~/Downloads/CLAUDE.md ~/Projects/pydicom/
```
