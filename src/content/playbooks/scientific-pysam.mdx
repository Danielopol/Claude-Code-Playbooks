...
seoHook: "Your NGS pipeline needs to extract reads from specific regions, calculate coverage depth, and filter variants — but BAM and VCF files are binary formats that require specialized libraries to access. Pysam provides fast Python access to SAM/BAM/CRAM alignments and VCF/BCF variants, the essential toolkit for any NGS data processing pipeline."
targetAudience: "bioinformaticians building custom NGS analysis pipelines in Python, genomics researchers extracting alignments and computing coverage from BAM files, variant analysts parsing and filtering VCF files programmatically, sequencing core facilities developing quality control scripts for NGS data, computational biologists integrating alignment data with downstream analysis tools"
exampleUseCase: "\"Extract reads from our target regions and compute coverage statistics from the BAM file\" → Pysam workflow: BAM file indexing and region-specific read extraction, per-base and per-region coverage depth calculation, read quality filtering and duplicate flagging, VCF variant extraction and genotype parsing, and a coverage and variant summary report for the targeted regions"
category: "academic-research"
difficulty: "intermediate"
timeToSetup: "10 minutes"
author: "K-Dense AI"
sourceUrl: "https://github.com/K-Dense-AI/claude-scientific-skills/tree/main/scientific-skills/pysam"
tags: ["scientific", "claude-code", "pysam", "bioinformatics", "visualization", "database", "genomics", "writing"]
...
```bash
mv ~/Downloads/CLAUDE.md ~/Projects/pysam/
```
...
- **Read the docs**: Check the official pysam documentation for latest API changes
