---
title: "Scientific Reactome Database"
...
seoHook: "Your gene list needs biological pathway context but KEGG requires a license for commercial use. Reactome is the largest open-source curated pathway database, with a REST API for pathway enrichment, gene-pathway mapping, and disease pathway analysis — free for any use."
targetAudience: "systems biologists running pathway enrichment analysis on gene expression data, cancer researchers mapping mutations to disease-specific signaling pathways, pharmacologists studying drug mechanism of action through pathway interaction networks, computational biologists building pathway-level features for machine learning models, translational researchers linking omics findings to biological mechanisms"
exampleUseCase: "\"Run pathway enrichment on our differentially expressed genes and identify affected signaling cascades\" → Reactome workflow: gene list submission for pathway enrichment analysis, statistically significant pathway identification with FDR correction, pathway hierarchy visualization showing affected parent-child relationships, molecular interaction network exploration for top pathways, and a pathway report with enrichment statistics and pathway diagrams"
category: "academic-research"
difficulty: "beginner"
timeToSetup: "5 minutes"
author: "K-Dense AI"
sourceUrl: "https://github.com/K-Dense-AI/claude-scientific-skills/tree/main/scientific-skills/reactome-database"
tags: ["scientific", "claude-code", "reactome-database", "chemistry", "statistics", "database", "protein", "genomics"]
...
```bash
mkdir -p ~/Projects/reactome-database
```
...
```bash
mv ~/Downloads/CLAUDE.md ~/Projects/reactome-database/
```

### Step 3: Start Claude Code

```bash
cd ~/Projects/reactome-database
claude
```
...
- **Read the docs**: Check the official reactome-database documentation for latest API changes
