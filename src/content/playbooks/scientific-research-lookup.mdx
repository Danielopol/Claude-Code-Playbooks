---
title: "Scientific Research Lookup"
description: "Look up current research information using the Parallel Chat API (primary) or Perplexity sonar-pro-search (academic paper searches). Automatically routes queries to the best backend. Use for finding papers, gathering research data, and verifying s..."
seoHook: "You need to quickly verify a scientific claim, find the latest paper on a method, or gather background for a proposal — but academic database searches return hundreds of results you don't have time to read. AI-powered research lookup synthesizes current findings from academic sources into concise, cited answers."
targetAudience: "researchers needing quick answers to specific scientific questions during active work, grant writers gathering recent citations and findings for proposal background sections, lab scientists verifying experimental methods and conditions from published literature, science journalists fact-checking claims against primary research sources, students looking up concepts and finding key references for coursework"
exampleUseCase: "\"Find the latest methods for single-cell spatial transcriptomics and their trade-offs\" → Research lookup: AI-synthesized summary of current spatial transcriptomics methods, comparison of key approaches with strengths and limitations, recent benchmark papers with performance metrics, links to primary sources for deeper reading, and a formatted summary ready for inclusion in a grant or review"
category: "research-writing"
difficulty: "beginner"
timeToSetup: "5 minutes"
author: "K-Dense AI"
sourceUrl: "https://github.com/K-Dense-AI/claude-scientific-skills/tree/main/scientific-skills/research-lookup"
tags: ["scientific", "claude-code", "research-lookup", "machine-learning", "statistics", "visualization", "database", "protein"]
...
```bash
mkdir -p ~/Projects/research-lookup
```
...
```bash
mv ~/Downloads/CLAUDE.md ~/Projects/research-lookup/
```

### Step 3: Start Claude Code

```bash
cd ~/Projects/research-lookup
claude
```
...
- **Read the docs**: Check the official research-lookup documentation for latest API changes
