...
seoHook: "Your multi-batch single-cell dataset has severe batch effects that simple regression can't remove without destroying biological signal. scVI uses deep generative models for probabilistic batch correction, differential expression with uncertainty estimates, and multi-modal integration — the state-of-the-art for complex single-cell analysis."
targetAudience: "single-cell biologists integrating datasets across batches and technologies with minimal overcorrection, computational biologists performing probabilistic differential expression with uncertainty quantification, multi-omics researchers integrating scRNA-seq with CITE-seq or ATAC-seq using TOTALVI and MultiVI, atlas-building teams harmonizing single-cell datasets from multiple labs, methods developers building on deep generative models for new single-cell applications"
exampleUseCase: "\"Integrate our multi-batch CITE-seq dataset with batch correction and protein-RNA joint analysis\" → scVI-tools pipeline: TOTALVI model training for joint RNA and protein integration, probabilistic batch effect removal preserving biological variation, differential expression with posterior uncertainty estimates, transfer learning to annotate new samples from trained model, and latent space visualization showing integrated cell populations"
category: "academic-research"
difficulty: "advanced"
timeToSetup: "15 minutes"
author: "K-Dense AI"
sourceUrl: "https://github.com/K-Dense-AI/claude-scientific-skills/tree/main/scientific-skills/scvi-tools"
tags: ["scientific", "claude-code", "scvi-tools", "bioinformatics", "machine-learning", "statistics", "database", "protein"]
...
```bash
mv ~/Downloads/CLAUDE.md ~/Projects/scvi-tools/
```

### Step 3: Start Claude Code

```bash
cd ~/Projects/scvi-tools
claude
```
