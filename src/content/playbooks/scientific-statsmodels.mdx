...
seoHook: "Scikit-learn gives predictions but not p-values, confidence intervals, or diagnostic plots — the things reviewers demand in a paper. Statsmodels provides R-style regression output with coefficient tables, residual diagnostics, and formal statistical inference for OLS, GLM, mixed models, and time series."
targetAudience: "econometricians fitting regression models with proper inference and diagnostic output, biostatisticians running mixed effects models for longitudinal and clustered data, time series analysts fitting ARIMA and state space models for forecasting, epidemiologists building logistic and Poisson regression models for risk analysis, social scientists needing coefficient tables with standard errors and p-values for publication"
exampleUseCase: "\"Fit a mixed effects model to our longitudinal patient data and check model diagnostics\" → Statsmodels workflow: mixed effects model specification with random intercepts and slopes, model fitting with REML estimation, coefficient table with confidence intervals and p-values, residual diagnostic plots for assumption checking, and model comparison using AIC/BIC for competing specifications"
category: "data-reporting"
difficulty: "intermediate"
timeToSetup: "10 minutes"
author: "K-Dense AI"
sourceUrl: "https://github.com/K-Dense-AI/claude-scientific-skills/tree/main/scientific-skills/statsmodels"
tags: ["scientific", "claude-code", "statsmodels", "statistics", "visualization", "database", "protein", "genomics"]
...
```bash
mv ~/Downloads/CLAUDE.md ~/Projects/statsmodels/
```

### Step 3: Start Claude Code

```bash
cd ~/Projects/statsmodels
claude
```
