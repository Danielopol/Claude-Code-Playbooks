---
title: "Scientific String Database"
...
seoHook: "Your proteomics experiment identified 200 differentially expressed proteins but you need to understand how they interact and which pathways they're involved in. STRING contains 20 billion protein-protein interactions across 5,000+ species — the go-to database for building interaction networks and running functional enrichment."
targetAudience: "systems biologists building protein interaction networks from gene lists, proteomics researchers placing differentially expressed proteins in functional context, cancer biologists identifying protein interaction hubs and network modules in disease, computational biologists integrating PPI data with expression changes for pathway analysis, drug discovery scientists mapping drug targets within interaction networks"
exampleUseCase: "\"Build an interaction network from our differentially expressed proteins and identify functional clusters\" → STRING workflow: protein list query for known and predicted interactions, network construction with confidence score filtering, functional enrichment analysis (GO terms, KEGG pathways), network clustering for module detection, and interaction network visualization with enrichment annotations"
category: "academic-research"
difficulty: "beginner"
timeToSetup: "5 minutes"
author: "K-Dense AI"
sourceUrl: "https://github.com/K-Dense-AI/claude-scientific-skills/tree/main/scientific-skills/string-database"
tags: ["scientific", "claude-code", "string-database", "bioinformatics", "chemistry", "statistics", "visualization", "database"]
...
```bash
mkdir -p ~/Projects/string-database
```
...
```bash
mv ~/Downloads/CLAUDE.md ~/Projects/string-database/
```

### Step 3: Start Claude Code

```bash
cd ~/Projects/string-database
claude
```
...
- **Read the docs**: Check the official string-database documentation for latest API changes
