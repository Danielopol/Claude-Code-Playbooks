---
title: "Scientific Zinc Database"
...
seoHook: "Screening millions of compounds for drug discovery requires access to purchasable, drug-like molecules with 3D conformations ready for docking. ZINC provides programmatic access to 230M+ commercially available compounds with similarity searches, substructure filtering, and pre-computed 3D structures."
targetAudience: "computational chemists running virtual screening campaigns against protein targets, medicinal chemists searching for purchasable analogs of active hit compounds, drug discovery researchers building focused compound libraries for specific target classes, structural biologists finding ligands with 3D conformations for molecular docking, cheminformatics scientists analyzing chemical space coverage and diversity"
exampleUseCase: "\"Find purchasable analogs of our lead compound for SAR expansion\" → ZINC pipeline: SMILES-based similarity search against 230M compounds, Tanimoto filtering at 0.7+ threshold, 3D conformer retrieval for top hits ready for docking, vendor availability and pricing lookup, and ranked analog list with structural diversity analysis"
category: "academic-research"
difficulty: "beginner"
timeToSetup: "5 minutes"
author: "K-Dense AI"
sourceUrl: "https://github.com/K-Dense-AI/claude-scientific-skills/tree/main/scientific-skills/zinc-database"
tags: ["scientific", "claude-code", "zinc-database", "chemistry", "database", "protein", "genomics", "writing"]
...
```bash
mkdir -p ~/Projects/zinc-database
```
...
```bash
mv ~/Downloads/CLAUDE.md ~/Projects/zinc-database/
```

### Step 3: Start Claude Code

```bash
cd ~/Projects/zinc-database
claude
```
